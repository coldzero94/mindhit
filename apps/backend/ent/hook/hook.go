...
func (f HighlightFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HighlightMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HighlightMutation", m)
}
...
// The PageVisitFunc type is an adapter to allow the use of ordinary
// function as PageVisit mutator.
type PageVisitFunc func(context.Context, *ent.PageVisitMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PageVisitFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PageVisitMutation); ok {
		return f(ctx, mv)
	}
