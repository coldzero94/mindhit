...
func (_q *SessionQuery) QueryPageVisits() *PageVisitQuery {
...
// QueryHighlights chains the current query on the "highlights" edge.
func (_q *SessionQuery) QueryHighlights() *HighlightQuery {
	query := (&HighlightClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SessionQuery) QueryRawEvents() *RawEventQuery {
...
// QueryMindmap chains the current query on the "mindmap" edge.
func (_q *SessionQuery) QueryMindmap() *MindmapGraphQuery {
	query := (&MindmapGraphClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// the "page_visits" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SessionQuery) WithPageVisits(opts ...func(*PageVisitQuery)) *SessionQuery {
	query := (&PageVisitClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPageVisits = query
	return _q
}

// WithHighlights tells the query-builder to eager-load the nodes that are connected to
// the "highlights" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SessionQuery) WithHighlights(opts ...func(*HighlightQuery)) *SessionQuery {
	query := (&HighlightClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withHighlights = query
	return _q
}
...
func (_q *SessionQuery) WithRawEvents(opts ...func(*RawEventQuery)) *SessionQuery {
	query := (&RawEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithMindmap tells the query-builder to eager-load the nodes that are connected to
// the "mindmap" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SessionQuery) WithMindmap(opts ...func(*MindmapGraphQuery)) *SessionQuery {
	query := (&MindmapGraphClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withPageVisits; query != nil {
		if err := _q.loadPageVisits(ctx, query, nodes,
			func(n *Session) { n.Edges.PageVisits = []*PageVisit{} },
			func(n *Session, e *PageVisit) { n.Edges.PageVisits = append(n.Edges.PageVisits, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withHighlights; query != nil {
		if err := _q.loadHighlights(ctx, query, nodes,
			func(n *Session) { n.Edges.Highlights = []*Highlight{} },
			func(n *Session, e *Highlight) { n.Edges.Highlights = append(n.Edges.Highlights, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRawEvents; query != nil {
		if err := _q.loadRawEvents(ctx, query, nodes,
			func(n *Session) { n.Edges.RawEvents = []*RawEvent{} },
			func(n *Session, e *RawEvent) { n.Edges.RawEvents = append(n.Edges.RawEvents, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMindmap; query != nil {
		if err := _q.loadMindmap(ctx, query, nodes, nil,
			func(n *Session, e *MindmapGraph) { n.Edges.Mindmap = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SessionQuery) loadPageVisits(ctx context.Context, query *PageVisitQuery, nodes []*Session, init func(*Session), assign func(*Session, *PageVisit)) error {
	...
	query.withFKs = true
	query.Where(predicate.PageVisit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.PageVisitsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_page_visits
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_page_visits" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_page_visits" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *SessionQuery) loadHighlights(ctx context.Context, query *HighlightQuery, nodes []*Session, init func(*Session), assign func(*Session, *Highlight)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Highlight(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.HighlightsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_highlights
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_highlights" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_highlights" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *SessionQuery) loadRawEvents(ctx context.Context, query *RawEventQuery, nodes []*Session, init func(*Session), assign func(*Session, *RawEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.RawEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.RawEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_raw_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_raw_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_raw_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *SessionQuery) loadMindmap(ctx context.Context, query *MindmapGraphQuery, nodes []*Session, init func(*Session), assign func(*Session, *MindmapGraph)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.MindmapGraph(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.MindmapColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.session_mindmap
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_mindmap" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
