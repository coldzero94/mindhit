...
func (_c *UserCreate) SetSettings(v *UserSettings) *UserCreate {
...
func (_c *UserCreate) AddAiLogIDs(ids ...uuid.UUID) *UserCreate {
	_c.mutation.AddAiLogIDs(ids...)
	return _c
}

// AddAiLogs adds the "ai_logs" edges to the AILog entity.
func (_c *UserCreate) AddAiLogs(v ...*AILog) *UserCreate {
	ids := make([]uuid.UUID, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
